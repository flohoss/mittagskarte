...
const refresh = () => {
    Loading.show({
        message: 'Eine Aktualisierung für ' + props.restaurant.name + ' findet statt. Dies kann mehrere Minuten dauern...',
...
<template>
    <div class="flex q-gutter-x-sm" v-if="restaurant.id !== ''">
        <q-btn id="upload" :size="iconSize" flat round icon="fa-solid fa-upload" @click="upload = true">
            <q-tooltip>Menü hochladen</q-tooltip>
        </q-btn>
        <q-btn id="refresh" :size="iconSize" flat round icon="fa-solid fa-rotate-right" @click="confirmRefresh()">
