...
<template>
    <q-input v-model="search" style="max-width: 20rem" class="full-width" dense placeholder="Suchen" :autofocus="false" type="search" @keyup.esc="search = ''">
        <template v-slot:append>
            <q-icon size="xs" name="fa-solid fa-magnifying-glass" />
        </template>

        <q-menu :no-parent-event="true" :no-focus="true" v-model="show" fit anchor="bottom left" self="top left" class="q-pl-md">
            <q-list style="min-width: 15rem">
                <NavRestaurant v-for="restaurant in result" :key="restaurant.id" :restaurant="restaurant" :search="true" />
            </q-list>
        </q-menu>
    </q-input>
</template>
