 ...
 * docker compose run -e PB_EMAIL=mail -e PB_PASSWORD=password --entrypoint node --rm yarn /app/seed/seed.mjs
 */
...
function resolveThumbnail(thumbnailField) {
    // Strip the random suffix: "avada_gevrx1cgg2.webp" → "avada"
    const stem = thumbnailField.replace(/_[a-z0-9]+\.webp$/i, '');
    const candidate = join(root, 'thumbnails', `${stem}.webp`);
...
await client.collection('users').authWithPassword(PB_EMAIL, PB_PASSWORD);
...
for (const r of restaurants) {
    ...
    // 1. Create selectors (navigate) and collect their IDs
    const navigateIds = [];
    ...
    for (const id of navigateIds) {
        formData.append('navigate', id);
    }

    // 3. Attach thumbnail if available
    if (r.thumbnail) {
